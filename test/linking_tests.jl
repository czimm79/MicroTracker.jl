...
@testset "Particle data to linked data" begin
    ...
    batch_final_linked_data = cd(()->batch_particle_data_to_linked_data(test_translation_dict, test_linking_settings; save_to_csv=false),
        get_assets_path())
    @test size(batch_final_linked_data) == (3863, 31)
...
@testset "Trajectory clipping" begin
    ...
    @test find_trajectory_bounds(gdf[3]) == (61, 1941) # clip both sides
    ...
    @test after_snipping[end, :frame] == 1940  # last frame of particle 3
    
end

@testset "helper functions" begin
    test_translation_dict = Dict("f_Hz"=>(1, Int64), "B_mT"=>(2, Float64), "FPS"=>(3, Float64))
    @test MicroTracker.translation_dict_to_string(test_translation_dict) == "B_mT=(2, Float64), FPS=(3, Float64), f_Hz=(1, Int64)"
end